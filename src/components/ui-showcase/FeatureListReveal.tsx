...
const FeatureListReveal = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (headingRef.current) {
        gsap.fromTo(headingRef.current, { yPercent: 100 }, {
          yPercent: 0, duration: 0.7, ease: 'power4.out',
        });
      }
      if (descRef.current) {
        gsap.fromTo(descRef.current, { opacity: 0, y: 10 }, {
      ...
      rowsRef.current.forEach((row, i) => {
        if (!row) return;
        gsap.fromTo(row, { opacity: 0, x: -24 }, {
          opacity: 1, x: 0, duration: 0.5, delay: 0.4 + i * 0.08, ease: 'power2.out',
        });
      });
      ...
      linesRef.current.forEach((line, i) => {
        ...
        gsap.fromTo(line, { scaleX: 0 }, {
          scaleX: 1, duration: 0.5, delay: 0.4 + i * 0.08, ease: 'power2.out',
  ...
  const handleRowEnter = (i: number) => {
    ...
    if (title) {
      ...
      gsap.to(title, { color: '#a78bfa', duration: 0.2 });
    }
  ...
  const handleRowLeave = (i: number) => {
    ...
    if (title) {
      ...
      gsap.to(title, { color: '#ededed', duration: 0.2 });
    }
  ...
  const handlePillEnter = () => {
    if (pillRef.current) gsap.to(pillRef.current, { scale: 1.03, duration: 0.2 });
  };
  const handlePillLeave = () => {
    if (pillRef.current) gsap.to(pillRef.current, { scale: 1, duration: 0.2 });
  };
    ...
    <div ref={containerRef} className="w-full" style={{ background: '#0a0a12', padding: '48px 40px', minHeight: 480 }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <div className="flex justify-between items-end" style={{ marginBottom: 48 }}>
          <div>
            <span
              ...
              style={{ fontSize: 10, color: '#a78bfa', letterSpacing: '0.2em', marginBottom: 12 }}
            >
              FEATURES
            </span>
            <div className="overflow-hidden">
              <div ref={headingRef}>
                <h2 className="font-syne font-extrabold" style={{ fontSize: '2.8rem', color: '#ededed', lineHeight: 1.1 }}>
                  What you get.
                </h2>
              </div>
            </div>
          ...
          <p
            ...
            style={{ fontSize: 13, color: '#606070', lineHeight: 1.7, maxWidth: 240, textAlign: 'right' }}
          >
        ...
        <div>
          {features.map((f, i) => (
            ...
            >
              ...
              {/* Background wipe */}
              <div
                className="fl-wipe absolute inset-0"
                style={{ background: '#13131e', zIndex: 0, transform: 'scaleX(0)', transformOrigin: 'left center' }}
              />
              {/* Row content */}
              <div className="relative flex items-center" style={{ zIndex: 1 }}>
                <span className="fl-num font-mono" style={{ width: '8%', fontSize: 11, color: '#303040' }}>
                  {f.num}
                </span>
                <span className="fl-title font-syne font-semibold" style={{ width: '62%', fontSize: '1.15rem', color: '#ededed' }}>
                  {f.title}
                </span>
                <span className="fl-tag font-mono" style={{ width: '30%', fontSize: 11, color: '#505060', textAlign: 'right', opacity: 0.4 }}>
                  {f.tag}
                </span>
              </div>
        ...
        <div
          ...
          style={{ marginTop: 32 }}
        >
          <span className="font-mono" style={{ fontSize: 11, color: '#303040' }}>
