...
const buildChartPath = (pts: number[], w: number, h: number) =>
  pts
    .map(
      (p, i) =>
        `${i === 0 ? 'M' : 'L'} ${(i / (pts.length - 1)) * w} ${h - (p / 100) * h}`,
    )
    .join(' ');
...
const StatsShowcase = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set(barRefs.current.filter(Boolean), {
        scaleX: 0,
            ...
            {[
              ...
              <div key={b.label} style={{ marginBottom: 6 }}>
                ...
                >
                  ...
                  <span
                    className="font-mono"
                    style={{ fontSize: '8px', color: b.color }}
                  >
                    {b.w}kb
                  </span>
                </div>
