...
interface CurtainPreloaderProps {
  ...
  /** Internal — used by the component card preview only. Do not set in your app. */
  _previewMode?: boolean;
}

// Non-linear steps — mimics real asset-loading rhythm (fast start, decelerates)
const STEPS: number[] = [
  0, 4, 9, 15, 23, 31, 40, 52, 61, 70, 78, 85, 91, 96, 100,
];
  ...
  return (
    <div
      style={{
        position: 'relative',
        width: '100%',
        minHeight: _previewMode ? '100%' : undefined,
        height: _previewMode ? '100%' : undefined,
      }}
    >
      {/* Overlay: fixed in real apps (true fullscreen), absolute in preview card */}
      <div
        ref={overlayRef}
        style={{
          position: _previewMode ? 'absolute' : 'fixed',
          inset: 0,
          zIndex: 9999,
  ...
  return (
    ...
    >
      ...
      <div data-item style={{ opacity: 0, maxWidth: 320 }}>
        ...
        >
          Counts to 100 then splits — top panel up, bottom panel down —
          unveiling your content in a single dramatic motion.
        </p>
      </div>
